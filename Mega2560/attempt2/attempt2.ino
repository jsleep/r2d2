...
void setup() {
  ...
  Serial.begin(115200);
  println(Serial.list());
  Serial port = new Serial(this, Serial.list()[2], 115200);
...
void loop() {
  ...
  byte key_command, option, mood, stop_track, song[3];
    ...
    if(option=='a') { 
      ...
      if(mood=='a') {
        ...
        while(stop_track!='s') {
          ...
          MP3player.playTrack(random(1,10));
          stop_track=Serial.read();
          if(stop_track=='s') {
            MP3player.stopTrack();
          }
        }
...
//code incomplete here, i am filing sounds before i copy it down. It will be the same as mood a
      }
      else if(mood=='c') {
      }
      else if(mood=='d') {
